...
SELECT SUM(CASE when m.id is null then 1 else 0 end ) AS id_null,
...
SELECT month(date_published) AS month_num, count(1) AS number_of_movies
FROM movie
...
SELECT 
  SUM(genre_count) AS single_genre_count
FROM (SELECT
  movie_id,
  COUNT(genre) AS genre_count
FROM genre
GROUP BY movie_id
HAVING genre_count = 1) AS a;
...
WHERE m.country='USA' 
	AND r.total_votes>1000 AND MONTH(date_published)=3 AND year=2017
GROUP BY g.genre
...
SELECT production_company, SUM(total_votes) AS vote_count,
		DENSE_RANK() OVER(ORDER BY SUM(total_votes) DESC) AS prod_rank_company
FROM movie AS m
INNER JOIN ratings AS r
ON m.id = r.movie_id
GROUP BY production_company
